...
async fn claim_is_atomic() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let epoch = storage
        .claim_and_persist_signature(id, token, "sig-claim".to_string(), 555)
        .await?
    ...
    let failed = storage
        .claim_and_persist_signature(id, stale, "sig-fail".to_string(), 1)
        .await?;
...
async fn claim_dedups_signature_on_conflict() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let token2 = storage
        .claim_and_persist_signature(id, token1, "dup-sig".to_string(), 1)
        .await?
    ...
    assert!(storage
        .claim_and_persist_signature(id, token2, "dup-sig".to_string(), 999)
        .await?
        .is_some());
    assert_eq!(
        storage.get_release_signatures(id).await?.len(),
