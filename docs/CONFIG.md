...
2. **Settlement state**: Reads the highest block row from PostgreSQL and resumes block production from it, taking the slot and the blockhash from that one row so they always describe the same block. Failure here is fatal - the node will not start when the tip cannot be read, because treating an unreadable tip as an empty ledger would restart a live chain from slot 0.
