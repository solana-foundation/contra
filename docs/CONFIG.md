...
2. **Settlement state**: Queries `latest_slot`, `block_height` and `latest_blockhash` from the database to resume block production from the correct point. Both counters are written inside the block transaction, and fall back to the last stored block when absent, so a node upgrading in place is correct from the first tick with no migration step. The idle ticks since the last block are not durable, so the slot resumes from that block rather than from the last tick.
