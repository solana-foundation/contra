# Contra Operator Runbook (single host)

Single-host deployment automation for Contra. Wraps `docker-compose.yml` so the deploy is one command, repeatable, and verifies itself.
...
3. **Build and push from the control node.** Set `image_registry` and `image_tag` in [`vars/dev.yml`](./vars/dev.yml) (e.g. `image_registry: ghcr.io/<your-github-username-lowercase>`, `image_tag: v0.1.0`), then run the snippet below from inside the `contra-deploy/` directory.
   ...
   docker build \
     ...
     -t ghcr.io/$OWNER/contra-app:$TAG ..
   docker build \
     --build-arg SOLANA_VERSION="$SOLANA_VERSION" \
     --build-arg YELLOWSTONE_TAG="$YELLOWSTONE_TAG" \
     -t ghcr.io/$OWNER/contra-validator:$TAG -f ../validator.Dockerfile ..
   docker push ghcr.io/$OWNER/contra-app:$TAG
   docker push ghcr.io/$OWNER/contra-validator:$TAG
   ```

   Image names must match what the compose stack expects: `<image_registry>/contra-app:<image_tag>` and `<image_registry>/contra-validator:<image_tag>`.
...
3. **Operator backlog depth** — `contra_operator_backlog_depth` per program, ≤ `OPERATOR_BACKLOG_MAX` (default 100).
4. **Yellowstone gRPC stability** — `contra_indexer_datasource_reconnects_total` snapshot, wait 10 s, re-snapshot; PASS if unchanged.
...
6. **Operator feepayer balance** — `contra_feepayer_balance_lamports` ≥ `FEEPAYER_MIN_LAMPORTS` (default 0.1 SOL).
7. **Pipeline movement** — `contra_dedup_received_total` sampled twice over 10 s; PASS if incrementing, SKIP if zero (no traffic on a fresh deploy is expected).
8. **Reconciliation log signals** — last 10 min of `contra-operator-solana` logs; FAIL on `MismatchExceedsThreshold`, PASS on recent reconciliation-OK lines, SKIP if neither.
...
Default-on (skip with `--skip-tags monitoring`). PHASE 6 brings up Prometheus + Grafana + cAdvisor + node_exporter + postgres_exporter + blackbox-exporter on the contra Docker network via a sibling `monitoring.compose.yml`.
